...
router.get('/create', (req, res) => { res.render('games/create') });
...
function getErrorMessage(error) {
    ...
    if (errorsArr.length > 0) { return error.errors[errorsArr[0]]; } else { return error.message }
};
...
async function isOwner(req, res, next) {
    ...
    if (games.owner == req.user._id) { res.redirect(`/games/details/${req.params.gameId}`); } else { next(); }
};
...
async function checkIsOwner(req, res, next) {
    let games = await gameService.getOne(req.params.gameId);
    if (games.owner == req.user._id) { next(); } else { res.redirect(`/games/details/${req.params.gameId}`); }
}
